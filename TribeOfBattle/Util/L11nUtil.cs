...
internal static class L11nUtil {
	private const string resPrefix = "TribeOfBattle.Resources.Lang.";
	...
	internal static readonly Lazy<Dictionary<string, Lazy<Dictionary<string, string>>>> Dict = new(() => Assembly
		...
		.ToDictionary(
			...
			tuple => new Lazy<Dictionary<string, string>>(() => {
				...
				Logger.LogDebug($"[{nameof(TribeOfBattle)}] - Loaded localization for lang: {tuple.lang}");
				return table;
			})
		)
	);
	...
	internal static string Localize(this string key) {
		Dict.Value.TryGetValue(CurrentLang, out Lazy<Dictionary<string, string>>? table);
